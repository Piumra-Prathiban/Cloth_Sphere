...
public class FabricRepository {
    ...
    public int updateFabric(Fabric fabric) {
        String sql = "UPDATE fabric SET current_stock = :stock, updated_at = :updatedAt WHERE fabric_id = :fabricId";
        MapSqlParameterSource params = new MapSqlParameterSource()
                .addValue("stock", fabric.getCurrentStock())
                .addValue("updatedAt", LocalDateTime.now()) // Use parameter instead of GETDATE()
                .addValue("fabricId", fabric.getFabricId());
        return namedJdbcTemplate.update(sql, params);
    }
